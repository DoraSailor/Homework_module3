...
# Задача: на основе наблюдаемых точек построить прямую, которая
# отображает связь между двумя и более переменными.abs
# Регрессия пытается "подогнать" некоторую функцию к наблюдаемым данным,
# чтобы спрогнозировать новые данные.

# Линейная регрессия:
# 1. Подгоняем данные к прямой линии;
# 2. Пытаемся установить линейную связь между переменными;
# 3. Пытаемся предсказать новые данные.
...
# n = len(x)
# w_1 = (n * sum(x[i] * y[i] for i in range(n)) - sum(x[i] for i in range(n))
#        * sum(y[i] for i in range(n)))/(n * sum(x[i] ** 2 for i in range(n))
#                                        - sum(x[i] for i in range(n)) ** 2)
# w_0 = ((sum(y[i] for i in range(n)) / n) - w_1 *
